...
public abstract class AssesmentReportFacadeBean implements SessionBean {
     ...
     * @ejb.permission role-name = "administrator,clientreporter,cepareporter,systemaccess"
     */
    public AssessmentReportData getAssessmentReportByCedi(Integer assessment, Integer cedi,  UserSessionData userSessionData) throws Exception {
    	AssessmentReportData reportData = new AssessmentReportData();
    	try {
    		AssesmentReport assessmentReport = AssesmentReportUtil.getHome().create();
    		reportData.setAssessment(assessmentReport.findAssesment(assessment, userSessionData));
    		...
    		UsReport userReport = UsReportUtil.getHome().create();
    		Collection userValues = userReport.getUsersReportByCedi(assessment,cedi,userSessionData);
    		HashMap<String, HashMap<Integer, String>> wrtAswers = userReport.getWRTUserAnswersByCedi(assessment, cedi,userSessionData);
    		Iterator it = userValues.iterator();
    		HashMap<String,UserReportData> mapUsers = new HashMap<String,UserReportData>();
    		HashMap<Integer,ModuleReportData> mapModules = new HashMap<Integer,ModuleReportData>();
    		while(it.hasNext()) {
    		...
    		QuestionReport questionReport = QuestionReportUtil.getHome().create();
    		Collection questionValues = questionReport.getQuestionReportByCedi(assessment,cedi,userSessionData);
    		it = questionValues.iterator();
     		while(it.hasNext()) {
     			Object[] data = (Object[])it.next();
     			mapModules.get(data[0]).addQuestion(data);
 ...
 public CategoryData findCategory(Integer id, UserSessionData userSessionData) throws Exception {
   ...
   */
    ...
    private boolean isNewGroup(int groupId) {
		return groupId == GroupData.ACHE || 
				groupId == GroupData.MERCADOLIBRE || 
				groupId == GroupData.MERCADOLIVRE || 
				groupId == GroupData.ECOLAB || 
				groupId >= 50;
	}
    /**
     * @ejb.interface-method 
	...
	public String[] getWebinarAdvance(String wbCode, String assesmentId, String login, UserSessionData userSessionData) throws Exception {
	  ...
	  public HashMap<String, HashMap<Integer, Object[]>> getUserSpecificQuestion(String questions, String login, UserSessionData userSessionData) throws Exception {
		...
		public Collection findMutualAssesmentResults(Integer assesment,Integer cedi, String from, String to, UserSessionData userSessionData) throws Exception {
	        try {
	            return AssesmentReportUtil.getHome().create().findMutualAssesmentResults(assesment,cedi, from, to,userSessionData);
	        }catch (Exception e) {
		...
		public Collection findGuinezAssesmentResults(Integer assesment,String division, String from, String to, UserSessionData userSessionData) throws Exception {
	        try {
	            return AssesmentReportUtil.getHome().create().findGuinezAssesmentResults(assesment,division, from, to,userSessionData);
	        }catch (Exception e) {
	            handler.handleException("findGuinezAssesmentResults",e);
	        }
	        return null;
	    }
	     ...
	     * @ejb.permission role-name = "administrator,clientreporter,cepareporter,systemaccess"
	     */
	    public AssessmentReportData getAssessmentReportBKP(Integer assessment, UserSessionData userSessionData) throws Exception {
	    	AssessmentReportData reportData = new AssessmentReportData();
	    	try {
	    		AssesmentReport assessmentReport = AssesmentReportUtil.getHome().create();
	    		reportData.setAssessment(assessmentReport.findAssesmentbkp(assessment, userSessionData));
	    		reportData.setAnswerTestCount(assessmentReport.getAssesmentQuestionCountbkp(reportData.getAssessment(), userSessionData, false));
	    		
	    		UsReport userReport = UsReportUtil.getHome().create();
	    		Collection userValues = userReport.getUsersReportBKP(assessment,userSessionData);
	    		HashMap<String, HashMap<Integer, String>> wrtAswers = userReport.getWRTUserAnswersBKP(assessment, userSessionData);
	    		Iterator it = userValues.iterator();
	    		HashMap<String,UserReportData> mapUsers = new HashMap<String,UserReportData>();
	    		HashMap<Integer,ModuleReportData> mapModules = new HashMap<Integer,ModuleReportData>();
	    		while(it.hasNext()) {
	    		...
	    		QuestionReport questionReport = QuestionReportUtil.getHome().create();
	    		Collection questionValues = questionReport.getQuestionReportBKP(assessment,userSessionData);
	    		it = questionValues.iterator();
	     		while(it.hasNext()) {
	     			Object[] data = (Object[])it.next();
	     			mapModules.get(data[0]).addQuestion(data);
	    		...
	    		Collection userNotStarted = userReport.getNotStartedUsersReportBKP(assessment,userSessionData);
	    		Iterator itNotStarted = userNotStarted.iterator();
	    		while(itNotStarted.hasNext()) {
	    			Object[] data = (Object[])itNotStarted.next();
	    			reportData.addNotStartedUser(new UserReportData((String)data[0],(String)data[1],(String)data[2],(String)data[3],null,true));
