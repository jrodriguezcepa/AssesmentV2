...
public class NewHirePersonalDataReportDataSource implements JRDataSource {

    private boolean index = true;
    private String[][] texts;
    private Text messages;
    
    public NewHirePersonalDataReportDataSource(ModuleData module, Collection answers, Text messages) {
    ...
    public Object getFieldValue(JRField field) throws JRException {
        if(field.getName().startsWith("value")) {
            int id = Integer.parseInt(field.getName().substring(5)) - 1; 
            if(id < texts.length) {
                return texts[id][1];
            }
        }
