...
public class GenerateReport {
	...
	public GenerateReport(Text messages, String language) {
		this.messages = messages;
		this.language = language;
	}
	
	public void getReport(String id, int assessment, int type, String fileName) {
		try {
			String user = "tmc_"+id;
	        ...
	        if(type == 1) {
	        	createTotalReport(enddate, messages, st1, user, new Integer(assessment), assessmentName, userName, moduleCount, psi, corporationId, fileName);
	        }else {
	        	createCertificate(enddate, messages,  user, assessment, userName, certificateImage, fileName);
	        }
    ...
    public void createTotalReport(Calendar enddate, Text messages, Statement st, String user, Integer assessment, String assessmentName, String userName, int moduleCount, boolean psi, String corporationId, String fileName) throws Exception {
		...
		ResultSet set = st.executeQuery("SELECT id, text FROM modules m join generalmessages gm on gm.labelkey = m.key WHERE gm.language = '"+ language +"' AND assesment = "+assessment);
		int index = 0;
		while(set.next()) {
		...
		params.put("fecha", dateToString(messages,enddate,language));
    ...
    public void createCertificate(Calendar enddate,Text messages, String user, Integer assessmentId, String userName, String certificateImage, String fileName) throws Exception {
    	HashMap<String,Object> parameters = new HashMap<String,Object>();
        parameters.put("User",userName);
        parameters.put("DateText",dateToString(messages,enddate,language));
...
//	        	JasperCompileManager.compileReportToFile("C:\\flash\\jasper\\certificadoFresenius.jrxml", "C:\\flash\\jasper\\certificadoFresenius.jasper");
	        ...
	        if(assessmentId == AssesmentData.ALLERGAN_2) {
	        	...
	        	parameters.put("Background",AssesmentData.FLASH_PATH+"/images/certificate_"+language+".jpg");
