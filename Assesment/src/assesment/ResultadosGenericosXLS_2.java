...
public class ResultadosGenericosXLS_2 {
    ...
    public static void main(String[] args) {
    	...
    	try {
            ...
            String modules = "0";
            ResultSet setModules = st.executeQuery("select id,type from modules where assesment = "+assesmentId+" ORDER BY moduleorder");
            while(setModules.next()) {
            	if(moduleDatosPersonales == null && setModules.getInt("type") == 1) {
	            	...
	            	for(i = 0; i < answers.length; i++) {
	            		...
	            		case QuestionData.TEXTAREA: case QuestionData.TEXT: case QuestionData.EMAIL:// Texto Simple            				
	            			...
	            			case QuestionData.INCLUDED_OPTIONS:
	            				...
	            				String v = "";
	            				if(set.next()) {
	            					v = set.getString("text");
		            				while(set.next()) {
		            					v += ", "+set.getString("text");
		            				}
	            				}
            					s.addCell(new Label(column, row,  v));
	            				break;
            ...
            while(set.next()) {
    	        ...
    	        while(it.hasNext()) {
    	        	...
    	        	if(Integer.parseInt(module) == 1297) {
	    	        	ResultSet set2 = st2.executeQuery("SELECT q.id,ad.text FROM useranswers ua " +
	    	        			"JOIN answerdata ad ON ua.answer = ad.id " +
	    	        	...
	    	        	while(set2.next()) {
	                		...
	                		s.addCell(new Label(map.get(String.valueOf(question)).intValue(), row,  set2.getString("text")));
	                		column++;
	    	        	}
    	        		
    	        	}else {
	    	        	ResultSet set2 = st2.executeQuery("SELECT q.id,a.key,ad.text FROM useranswers ua " +
	    	        			"JOIN answerdata ad ON ua.answer = ad.id " +
	    	        			"JOIN questions q ON q.id = ad.question " +
	    	        			"JOIN answers a ON a.id = ad.answer " +
	    	        			"WHERE ua.assesment = " + assesmentId + " "+
	    	        	...
	    	        	while(set2.next()) {
	    	        		...
	    	        		if(question == 19535) {
	    	        			...
	    	        			ResultSet set3 = st3.executeQuery("SELECT text FROM generalmessages WHERE labelkey = '" +set2.getString("key")+"' AND language = '"+language+"'");
	    	        			set3.next();
	                			s.addCell(new Label(map.get(String.valueOf(question)).intValue(), row,  set3.getString("text")));
	                			column++;
	    	        		}
