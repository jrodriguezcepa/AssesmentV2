...
/**
	 ...
	 * @ejb.permission role-name = "administrator,clientreporter,cepareporter,systemaccess"
	 */
	public Collection getUsersReportBKP(Integer assessment, UserSessionData userSessionData) throws Exception {
		try {
			String filter = "";
			if(userSessionData.getFilter().getLoginName().startsWith("charla")) {
				filter = " AND u.loginname IN (SELECT loginname FROM usersaccess WHERE passworddate > NOW() - INTERVAL '2 hour' AND loginname LIKE 'generate_es_%_"+assessment+"') ";
			...
			String sql = "select u.loginname,u.firstname,u.lastname,u.email,ua.enddate,q.module,a.type,count(*) AS c,ua.psiresult1,ua.psiresult2,ua.psiresult3,ua.psiresult4,ua.psiresult5,ua.psiresult6 "
					+ "from userassesmentsbkp ua " + "join users u on u.loginname = ua.loginname "
					+ "join useranswersbkp uas on uas.loginname = ua.loginname and uas.assesment = ua.assesment "  
					+ "join answerdatabkp ad on ad.id = uas.answer " + "join questionsbkp q on ad.question = q.id "
					+ "join answersbkp a on ad.answer = a.id " + "where ua.assesment = " + assessment + filter
					+ " and q.testtype = " + QuestionData.TEST_QUESTION + " "
					+ "group by u.loginname,u.firstname,u.lastname,u.email,ua.enddate,q.module,a.type,ua.psiresult1,ua.psiresult2,ua.psiresult3,ua.psiresult4,ua.psiresult5,ua.psiresult6 "
					+ "UNION (select distinct u.loginname,u.firstname,u.lastname,u.email,ua.enddate,q.module,0 AS type,0,ua.psiresult1,ua.psiresult2,ua.psiresult3,ua.psiresult4,ua.psiresult5,ua.psiresult6 "
					+ "from users u " + "join userassesmentsbkp ua on u.loginname = ua.loginname "
					+ "join useranswersbkp uas on u.loginname = uas.loginname and uas.assesment = ua.assesment  "
					+ "join answerdatabkp ad on ad.id = uas.answer " + "join questionsbkp q on ad.question = q.id "
					+ "where ua.assesment = " + assessment + filter
					+ " and u.loginname not in (select loginname from useranswersbkp ua join answerdatabkp ad on ad.id = ua.answer join questionsbkp q on q.id = ad.question where ua.assesment = "
					+ assessment + " and q.testtype = " + QuestionData.TEST_QUESTION + ")) "
					+ "order by loginname,type";
			Session session = HibernateAccess.currentSession();
			SQLQuery question = session.createSQLQuery(sql);
			...
			handler.getException(e, "getUsersReportBKP", userSessionData.getFilter().getLoginName());
		}
		return new LinkedList();
	}
	 ...
	 * @ejb.permission role-name = "administrator,clientreporter,cepareporter,systemaccess"
	 */
	public HashMap<String, HashMap<Integer, String>> getWRTUserAnswersBKP(Integer assessment, UserSessionData userSessionData) throws Exception {
		HashMap<String, HashMap<Integer, String>> values = new HashMap<String, HashMap<Integer, String>>();
		try {
			String filter = "";
			if(userSessionData.getFilter().getLoginName().startsWith("charla")) {
			...
			String sql = "SELECT ua.loginname,ad.question,ad.text,ad.date,ad.distance, ad.unit, ad.country, ad.never, a.key "
					+ "FROM answerdatabkp ad " + "JOIN useranswersbkp ua ON ua.answer = ad.id "
					+ "JOIN questionsbkp q ON q.id = ad.question " + "LEFT JOIN answersbkp a ON a.id = ad.answer "
					+ "WHERE ua.assesment = " + assessment + filter + " AND q.wrt";
			Session session = HibernateAccess.currentSession();
			SQLQuery question = session.createSQLQuery(sql);
			question.addScalar("loginname", Hibernate.STRING);
			...
			handler.getException(e, "getWRTUserAnswersBKP", userSessionData.getFilter().getLoginName());
		}
		return values;
	}
	 ...
	 * @ejb.permission role-name = "administrator,clientreporter,cepareporter,systemaccess"
	 */
	public Collection getNotStartedUsersReportBKP(Integer assessment, UserSessionData userSessionData) throws Exception {
		try {
			String filter = "";
			if(userSessionData.getFilter().getLoginName().startsWith("charla")) {
				filter = " AND u.loginname IN (SELECT loginname FROM usersaccess WHERE passworddate > NOW() - INTERVAL '2 hour' AND loginname LIKE 'generate_es_%_"+assessment+"') ";
			...
			String sql = "select u.loginname,u.firstname,u.lastname,u.email " + "from userassesmentsbkp ua "
					+ "join users u on u.loginname = ua.loginname " + "where ua.assesment = " + assessment + filter 
					+ " and u.loginname not in (select distinct loginname from useranswersbkp where assesment = "
					+ assessment + ")";
			Session session = HibernateAccess.currentSession();
			SQLQuery question = session.createSQLQuery(sql);
			question.addScalar("loginname", Hibernate.STRING);
			...
			handler.getException(e, "getNotStartedUsersReportBKP", userSessionData.getFilter().getLoginName());
		}
		return new LinkedList();
	}
	 ...
	 * @ejb.permission role-name = "administrator,clientreporter,cepareporter,systemaccess"
	 */
	public Collection getUsersReportByDivision(Integer assessment, String division, UserSessionData userSessionData) throws Exception {
		try {
			String filter = (division==null || division.equals("")) ?"":(" AND extradata='"+division+"'");
			String sql = "select u.loginname,u.firstname,u.lastname,u.email,ua.enddate,q.module,a.type,count(*) AS c,ua.psiresult1,ua.psiresult2,ua.psiresult3,ua.psiresult4,ua.psiresult5,ua.psiresult6 "
					+ "from userassesments ua " + "join users u on u.loginname = ua.loginname "
					+ "join useranswers uas on uas.loginname = ua.loginname and uas.assesment = ua.assesment "  
					+ "join answerdata ad on ad.id = uas.answer " + "join questions q on ad.question = q.id "
	 ...
	 * @ejb.permission role-name = "administrator,clientreporter,cepareporter,systemaccess"
	 */
	public HashMap<String, HashMap<Integer, String>> getWRTUserAnswersByDivision(Integer assessment,String division, UserSessionData userSessionData) throws Exception {
		HashMap<String, HashMap<Integer, String>> values = new HashMap<String, HashMap<Integer, String>>();
		try {
			String filter =  (division==null || division.equals("")) ?"":(" AND extradata='"+division+"'");
			String sql = "SELECT ua.loginname,ad.question,ad.text,ad.date,ad.distance, ad.unit, ad.country, ad.never, a.key "
			...
			handler.getException(e, "getWRTUserAnswersByDivision", userSessionData.getFilter().getLoginName());
		}
		return values;
	}
