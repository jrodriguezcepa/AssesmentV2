...
public class DownloadResultReportBKPAction  extends AbstractAction {
        	...
        	if(assessment != null) {
	        	if(Integer.parseInt(assessment) == AssesmentData.ANTP_MEXICO_RSMM) {
	        		...
	        		ResultSet setUsers = st.executeQuery("SELECT u.loginname, u.firstname, u.lastname, u.email, ua.enddate, min(uas.answer), COUNT(*) " +
	                		"FROM users u " +
	                		"JOIN userassesmentsbkp ua ON u.loginname = ua.loginname " +
	                		"LEFT JOIN useranswersbkp uas ON u.loginname = uas.loginname " +
	            	...
	            	if(rsmm != null) {
	            		ResultSet setAnswers = st.executeQuery("SELECT ua.loginname, m.moduleorder, q.questionorder, a.answerorder FROM useranswersbkp ua " +
	            			"JOIN answerdatabkp ad ON ad.id = ua.answer " +
	            			"JOIN answersbkp a ON a.id = ad.answer " +
	            			"JOIN questionsbkp q ON q.id = a.question " +
	        		...
	        		ResultSet setUsers = st.executeQuery("select enddate from userassesmentsbkp where loginname = '" + loginname + "'"); // cantidad total de usuarios
	        		setUsers.next();
	        		Date endDate = setUsers.getDate(1);
	        		
	        		setUsers = st.executeQuery("select text from useranswersbkp ua join answerdatabkp ad on ad.id = ua.answer " +
	        		...
	        		ResultSet setAnswers1 = st.executeQuery("select q.module, a.answerorder, count(*) from useranswersbkp ua " +
	        		"join answerdatabkp ad on ad.id = ua.answer " +
	        		...
	        		ResultSet setAnswers2 = st.executeQuery("select q.module, q.id from useranswersbkp ua " +
	        				"join answerdatabkp ad on ad.id = ua.answer " +
	        				"join questionsbkp q on ad.question = q.id " +
	        				"join answersbkp a on ad.answer = a.id " +
	        				"where loginname = '" + loginname + "' " +
    ...
    private String[] getUserData(Statement st, Integer assessment, String user, Text messages, String language) throws Exception {
    	...
    	String sql = "select enddate, firstname, lastname, a.name, c.name, a.id " + 
    			...
    			"join assesmentsbkp a on a.id = ua.assesment " + 
