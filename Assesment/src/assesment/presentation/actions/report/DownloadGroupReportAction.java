...
public class DownloadGroupReportAction  extends AbstractAction {
    ...
    public ActionForward action(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Throwable {
		try {
			...
			boolean grupoModelo = (group.getId().intValue() == GroupData.GRUPO_MODELO);

	        boolean mercadolivre = (group.getId().intValue() == GroupData.MERCADOLIVRE);
			boolean mercadolibre = (group.getId().intValue() == GroupData.MERCADOLIBRE);
	        ...
	        if(mercadolibre)
		        row.add(new Object[] {"CURP", new Short(new HSSFColor.GREY_25_PERCENT().getIndex())});
	        ...
	        if(mercadolivre)
		        row.add(new Object[] {"Companhia/MLP", new Short(new HSSFColor.GREY_25_PERCENT().getIndex())});
	        if(mercadolibre)
		        row.add(new Object[] {"Compañía/MLP", new Short(new HSSFColor.GREY_25_PERCENT().getIndex())});
	        if(grupoModelo) {
			...
			Integer[] cedis=null;
			HashMap<String, HashMap<Integer, Object[]>> usersData=null;
			if(grupoModelo) {
			...
			while(it.hasNext()) {
				...
				if(mercadolibre)
					rowUser.add(user.getExtraData2().toUpperCase());
				rowUser.add(user.getFirstName());
				rowUser.add(user.getLastName());
				rowUser.add(user.getEmail());
				if(grupoModelo) { 
					values2 = (usersData.containsKey(user.getLoginName())) ? usersData.get(user.getLoginName()) : new HashMap<Integer, Object[]>();
