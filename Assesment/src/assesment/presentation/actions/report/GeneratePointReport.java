...
import assesment.communication.report.ModulePointReportDataSource;
...
import assesment.communication.report.TotalPointReportDataSource;
...
public class GeneratePointReport {

	private AssesmentAccess sys;
	private String sendId;
	
	public GeneratePointReport() {
	}


	public GeneratePointReport(AssesmentAccess sys, String user, int assessment, int type) {
		try {
			this.sys = sys;
	    	...
	    	set = st1.executeQuery("select a.name, c.name, count(*), c.id, a.psitest, a.reportfeedback,a.certificate,a.certificateimage"+language+",a.green,a.yellow,a.dcactivity,c.dc5id from assesments a join corporations c on c.id = a.corporation join modules m on m.assesment = a.id where a.id = "+assessment+" group by a.name, c.name, c.id, a.psitest, a.reportfeedback,a.certificate,a.certificateimage"+language+",a.green,a.yellow,a.dcactivity,c.dc5id");
	    	set.next();
        	...
        	String sql = "select a.type, count(*), ua.enddate, ua.elearningredirect, sum(a.points) " +
        			"from userassesments ua, useranswers uas, answerdata ad, questions q, answers a " +
	...
	public GeneratePointReport(AssesmentAccess sys, String user, int assessment) {
		try {
			this.sys = sys;
	    	Class.forName("org.postgresql.Driver");
	    	Connection connDA = DriverManager.getConnection("jdbc:postgresql://localhost:5432/assesment","postgres","pr0v1s0r1A");
	    	...
	    	if(assessment == 425) {
	    		ResultSet set2 = st2.executeQuery("select text from useranswers ua " +
...
//        JRFileVirtualizer virtualizer = new JRFileVirtualizer(1, AssesmentData.FLASH_PATH+"/tmp");
 //       params.put(JRParameter.REPORT_VIRTUALIZER, virtualizer);
    ...
    public FileOutputStream createTotalReport(Calendar enddate, Text messages, Statement st, String fileName,String user, Integer assessment, String assessmentName, String userName, int moduleCount, boolean psi, String corporationId) throws Exception {
		...
		ResultSet set = st.executeQuery("SELECT id, text FROM modules m join generalmessages gm on gm.labelkey = m.key WHERE gm.language = '"+ sys.getUserSessionData().getLenguage() +"' AND assesment = "+assessment);
		int index = 0;
		while(set.next()) {
        ...
        String subreportName = (moduleCount > 4) ? "ModuleSubreport5.jasper" : "ModulePointSubreport.jasper";
        FileInputStream input =  new FileInputStream(new File(AssesmentData.JASPER_PATH +  subreportName));
        JasperReport moduleSubreport = (JasperReport)JRLoader.loadObject(input);
        ModulePointReportDataSource moduleDataSource = new ModulePointReportDataSource(map.size());
        ...
        for(int i = 0; i < moduleCount; i++) {
        	if(map.containsKey(new Integer(modules[i][0]))) {
        		...
        		points += (Float)data[3];
        		moduleDataSource.addModule(modules[i][1], (Integer)data[2], (Integer)data[1], (Float)data[3]);
        	}
        }
        
        
        TotalPointReportDataSource totalDataSource = new TotalPointReportDataSource(correctas,incorrectas,points,messages);
        ...
        if(moduleCount == 1) {
        	principalFileName = AssesmentData.JASPER_PATH+"TotalPointReport1.jasper";
        }
        if(moduleCount > 4) {
...
//	        	JasperCompileManager.compileReportToFile("C:\\flash\\jasper\\certificadoFresenius.jrxml", "C:\\flash\\jasper\\certificadoFresenius.jasper");
	...
	private Integer[] getUserPsicoResult(Statement st, String user, Integer assessment) throws Exception {
        ...
        String sql = "SELECT psiresult1,psiresult2,psiresult3," +
            "psiresult4,psiresult5,psiresult6 FROM userassesments " +
	...
	private HashMap findUsersResults(Statement st, Integer assessment, String user) throws Exception {
        ...
        String queryStr = "SELECT module,m.key,a.type,count(*) AS count, SUM(a.points) AS points " +
        ...
        while(set.next()) {
            ...
            Object[] dataResult =  {set.getString(2),0,0,new Float(0)};
            ...
            dataResult[3] = ((Float)dataResult[3]).floatValue() + set.getFloat(5);
        }
        if(!result) {
            ...
            while(set.next()) {
                Integer moduleId = set.getInt(1);
                Object[] dataResult =  {set.getString(2),0,0,0};
                if(hash.containsKey(moduleId)) {
                    dataResult =  (Object[])hash.get(moduleId);
                }else {
                    hash.put(moduleId,dataResult);
...
*/	
    ...
    private static String getUserAnswer(String user, int question, Statement st) throws Exception {
		ResultSet set = st.executeQuery("select a.key from answerdata ad join useranswers ua on ua.answer = ad.id join answers a on a.id = ad.answer " +
