...
public class BasfCreateUserAction extends AbstractAction {
    ...
    public ActionForward action(ActionMapping mapping, ActionForm createForm, HttpServletRequest request, HttpServletResponse response) throws Throwable {
    	try {
	        ...
	        Integer[] assessments = sys.getAssesmentReportFacade().getAvailableAssessments(null, CorporationData.BASF, userSessionData);
	        int x = (int)Math.round(Math.random()*(assessments.length-1));
	        Integer assessment = assessments[x];
	        
	        
	        String login = sys.getUserABMFacade().createBasfUser(assessment, null, userSessionData);
