...
<%@page import="assesment.persistence.administration.tables.UserMultiAssessment"%>
		...
		Enumeration enumeration = request.getParameterNames();
	 	if(enumeration.hasMoreElements())  {
			...
			if(module.equals("1079") && answerMap.containsKey(new Integer(17586))) {
	 		...
	 		if(userSessionData.getFilter().getAssesment().intValue() == AssesmentData.FRAYLOG_FATIGA) {
	 			cont = false;
	 			Object[] values = sys.getUserABMFacade().saveMultiAssessment(answerMap,new Integer(module),userSessionData);
				...
				String finalText = "";
				UserMultiAssessmentData answerData = sys.getAssesmentReportFacade().getMultiAssessment(userSessionData);
				if(answerData.getEndDate() == null) {
					Date d = sys.getUserReportFacade().isAssessmentDone(userSessionData);
					if(d != null) {
						answerData.setEndDate(d);
						AnswerUtil answerUtil = AnswerUtil.getInstance(answerData.getAssessment());
						UserData user = sys.getUserReportFacade().findUserByPrimaryKey(login, userSessionData);
						answerUtil.sendMultiEmail(sys, answerData, user);
					}
				}
				response.getWriter().print("]");
			...
			if(cont) {
				...
				if(answerMap.size() > 0 && !done) {
					...
					Object[] values = answerUtil.saveAnswers(sys,module,answerMap,true);
