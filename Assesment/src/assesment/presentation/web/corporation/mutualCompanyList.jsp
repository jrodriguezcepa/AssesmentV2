...
<%
	...
	if(check!=null) {
		...
		session.setAttribute("url","corporation/mutualCompanyList.jsp");
		...
		ListResult result = sys.getCorporationReportFacade().findCediName(value,CediData.MUTUAL,sys.getUserSessionData());
...
<head/>
	...
	<body>
		<jsp:include  page='<%="../component/titlecomponent.jsp?title="+messages.getText("system.data.mutualcompany")%>' />
	  		<tr>
		    		...
		    		<table width="100%" border="0" cellpadding="0" cellspacing="0">
						<tr>
							<td align="left" class="line">
								<form action="layout.jsp?refer=/corporation/mutualCompanyist.jsp" method="post">
									<%=messages.getText("generic.company")%>
	  		...
	  		<tr>
	    		<td colspan="3">
					<jsp:include  page='<%="../component/utilitybox2top.jsp?title="+messages.getText("system.data.mutualcompany")%>' />
...
%>          		</table>
			...
			<tr>
				<td colspan="3" align="right">
                	<input name="button2" type="button" onClick="javascript:deleteIFConfirm(document.forms['CediDeleteForm'],'<%=messages.getText("corporation.mutualcompany.delete.confirm")%>');" value='<%=messages.getText("generic.messages.delete")%>' class="button"/>
				</td>
           	</tr>
