...
<%
	...
	if(check!=null) {
		...
		session.setAttribute("url","question/tags.jsp?assesment="+assesment+"&question="+question);

	
		RequestDispatcher dispatcher=request.getRequestDispatcher("/util/jsp/message.jsp");
		dispatcher.include(request,response);

		Collection list = sys.getTagReportFacade().getListNotAssociatedQuestion(new Integer(assesment),new Integer(question),sys.getUserSessionData());
%>
	<head/>
	<html:form action="/AssociateQuestionTag">
...
%>							<tr class='<%=(lineone) ? "lineone" : "linetwo" %>'>
								...
								<th width="95%" align="left">
									<%=messages.getText("assesment.elearning.lesson"+String.valueOf(tag.getId()))%>
								</th>
<%			lineone = !lineone;
		}
%>							</tr>
					      	<tr>
					      		<td align="right" colspan="3">
						            <input type="button" class="input" value='<%=messages.getText("assessment.action.associate")%>' onClick="javascript:confirmAssociation(document.forms['AssociateQuestionTagForm'],'<%=messages.getText("assesment.associatetag.confirm")%>');" />
