...
<head/>
	...
	<body>
		...
		<jsp:include  page='<%="../component/titlecomponent.jsp?title="+messages.getText("generic.assesment")+" "+messages.getText(data.getName())%>' />
	  		<tr>			  		
				<jsp:include page='<%="viewOptions.jsp"+"?status="+String.valueOf(data.getStatus())%>' />
			</tr>
    				...
    				<table width="100%" border="0" align="center" cellpadding="2" cellspacing="2">
    					...
    					<tr>
    						<td width="100%" valign="top" colspan="2">
								<jsp:include  page='../component/utilitybox2top.jsp?title=Links' />
								<html:form action="/CreateAssessmentLink">
				    				...
				    				<table border="0" align="left" cellpadding="2" cellspacing="2">
				    					<tr class="line">
											...
											<td align="left">
<%						if(Util.empty(data.getLink_es())) {
%>			         							<html:text property="firstNameES" size="25" styleClass="input"/>
			  									<html:text property="lastNameES" size="25" styleClass="input"/>
			  									<input type="button" class="input" onclick="createLink('es')" value='<%=messages.getText("assessment.createlink")%>' />
<%						}else {
%>												<%=data.getLink_es()%>													
<%						}
%>				    						</td>
				    					...
				    					<tr class="line">
											...
											<td align="left">
<%						if(Util.empty(data.getLink_en())) {
%>			         							<html:text property="firstNameEN" size="25" styleClass="input"/>
			  									<html:text property="lastNameEN" size="25" styleClass="input"/>
			  									<input type="button" class="input" onclick="createLink('en')" value='<%=messages.getText("assessment.createlink")%>' />
<%						}else {
%>												<%=data.getLink_en()%>													
<%						}
%>	    									</td>
				    					...
				    					<tr class="line">
											...
											<td align="left">
<%						if(Util.empty(data.getLink_pt())) {
%>				       							<html:text property="firstNamePT" size="25" styleClass="input"/>
			    								<html:text property="lastNamePT" size="25" styleClass="input"/>
			    								<input type="button" class="input" onclick="createLink('pt')" value='<%=messages.getText("assessment.createlink")%>' />
<%						}else {
%>												<%=data.getLink_pt()%>													
<%						}
%>				    						</td>
										...
										<tr class="line">
											<td align="left">
    											<a href='<%="./layout.jsp?refer=/module/view.jsp?module="+String.valueOf(module.getId())%>'>
													<span style="color:#333333; font-size:18;"><%=" - "+messages.getText(module.getKey())%><span>
												</a>
											</td>
										</tr>
   										<tr>
...
%>
			...
			<tr>	
<%
        UsersReportDataSource dataSource = sys.getUserReportFacade().findUsersAssesment(new Integer(id),sys.getUserSessionData(),messages);
