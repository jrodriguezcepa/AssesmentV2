...
<html:html>
	...
	<body class="login">
		...
		<section id="content" style="padding-top: 50px;">
			...
			<form name="relogin" action="https://www.cepada.com/registrolumin" method="post">
				<fieldset id="username_block" class="active">
					...
					<div>
	                  	<label for="accesscode"><%=messages.getText("charlas.lumin.success")%></label>
					</div>
					<div>
	                  	<label for="accesscode"><%=userData.getFirstName()+" "+userData.getLastName()%></label>
					</div>
 					<div>
	                  	<label for="accesscode"><%=messages.getText("charlas.lumin.insert")+" "+userData.getExtraData()+"."%></label>
					</div>
					<div>
						<div>
			  				<input type="button" class="button" value='<%=messages.getText("charlalumin.messages.register")%>' onClick="javascript:document.forms['relogin'].submit()" />
						</div>
						<div>
