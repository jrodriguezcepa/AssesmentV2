	...
	<body class="login">
		...
		<section id="content" style="padding-top: 50px;">
			<section class="grid_container">
				<html:form action="/RegisterUPM" style="padding-right: 0px;">
					...
					<fieldset id="username_block" class="active">
						...
						<div>
		                  	<label for="accesscode"><%=messages.getText("charlas.upm.register")%></label>
						</div>
...
%>						<div>
						...
						<div>
							<div style="float: left; width: 250px;">
								<input type="button" class="button" style="width: 200px;" value="<%=messages.getText("generic.messages.register")%>" onclick="javascript:document.forms['CharlaUPMForm'].submit();"/>
							</div>
							<div>
...
%>						<div>
		                  	<label for="accesscode">
		                  		<span class="red">
		                  			<%=messages.getText("upm.charla.cinotfounded")%>
		                  		</span>
		                  	</label>
						</div>
						<div>
...
<%      	  	Connection connDC = (SecurityConstants.isProductionServer()) ? DriverManager.getConnection("jdbc:postgresql://18.229.182.37:5432/datacenter5","postgres","pr0v1s0r1A") : DriverManager.getConnection("jdbc:postgresql://localhost:5432/datacenter5","postgres","pr0v1s0r1A");
	        	...
	        	ResultSet set = stDC.executeQuery("SELECT resourcekey FROM divorgitemlevel1s WHERE active AND divorg = 10 ORDER BY resourcekey");
	        	while(set.next()) {
