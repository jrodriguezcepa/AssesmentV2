...
<%
	...
	String language = (sys != null) ? sys.getUserSessionData().getLenguage()
			: System.getProperty("user.language");
	String next = (sys.getUserSessionData().getRole().equals(SecurityConstants.ADMINISTRATOR)) ? "index.jsp"
			: "logout.jsp";
...
<body>
	<form action="logout.jsp">
		...
		<br> <br> <input type="submit" class="button"
			value='<%=Text.getMessage("generic.messages.accept")%>' /> <br>
	</form>
</body>
<%
	} else {
		...
		AssesmentData assesment = sys.getAssesmentReportFacade().findAssesment(new Integer(assesmentId),
				userSessionData);
		...
		if (cedi != null)
			session.setAttribute("cedi", cedi.getId());
		if (division != null)
		...
		Collection r = null;
		if (Integer.parseInt(assesmentId) == AssesmentData.GUINEZ_INGENIERIA_V3) {
			r = sys.getAssesmentReportFacade().findGuinezAssesmentResults(Integer.parseInt(assesmentId),
					division, date_from, date_to, sys.getUserSessionData());
		} else {
			r = sys.getAssesmentReportFacade().findMutualAssesmentResults(Integer.parseInt(assesmentId), cediId,
					date_from, date_to, sys.getUserSessionData());
		}
		HashMap globalResults = null;
		Set<String> keys = null;
		if (Integer.parseInt(assesmentId) == AssesmentData.MUTUAL_DA) {
			globalResults = sys.getAssesmentReportFacade().findMutualAssesmentGlobalResults(
					Integer.parseInt(assesmentId), cediId, sys.getUserSessionData());
			keys = globalResults.keySet();
		}
		if (Integer.parseInt(assesmentId) == AssesmentData.GUINEZ_INGENIERIA_V3) {
			globalResults = sys.getAssesmentReportFacade().findGuinezAssesmentGlobalResults(
					Integer.parseInt(assesmentId), division, sys.getUserSessionData());
			keys = globalResults.keySet();
		}
...
<body>
	<header class="header">
		...
		<div id="right">
			<div id="logout_user">
				<span> <%=user.getFirstName() + " " + user.getLastName()%></span> <a
					href="<%=next%>"> <img id="logout_icon"
					src="images/mutual_logout.png" alt="logout">
				</a>
			</div>
		</div>
	...
	<div>
		<table class="table3">
			...
			<tr>
				<th><%=comp%></th>
				<td style="color: #3995D2"><%=(Integer) ((Object[]) globalResults.get(comp))[0]%>
				</td>
				<td style="color: #3995D2"><%=(Integer) ((Object[]) globalResults.get(comp))[1]%>
				</td>
				<td style="color: #3995D2"><%=(Integer) ((Object[]) globalResults.get(comp))[2]%>
				</td>
				<td>
					<%
						if (comp.equals("STP")) {
					...
					%> <%=((Object[]) globalResults.get(comp))[3] == null ? "--"
									: (String) ((Object[]) globalResults.get(comp))[3]%>
					<%
						}
					%>
				</td>
 ...
 %>
	...
	<div class="row">
		<div class="col-1">
			<a href='<%=refresh%>' class="button"><%=messages.getText("generic.data.refresh")%>
				<img src="images/mutual_refresh.png" alt="left"></a>
		</div>
		<div class="col-2">
			<html:form action="/DownloadMutualReport" method="post">
				<a href="" class="button"><html:submit style="all:unset;"><%=messages.getText("generic.data.downloadxls")%>
					</html:submit><img src="images/mutual_download.png" alt="left"></a>
			</html:form>
		</div>
		<div class="col-3">
			<a href='<%=report + "&action=2"%>' class="button"><%=messages.getText("generic.data.generalresults")%></a>
		</div>
		<div class="col-4">
			<a href='<%=report + "&action=3"%>' class="button"><%=messages.getText("generic.data.questionsranking")%></a>
		</div>
		<div class="col-5">
			<a href='<%=report + "&action=4"%>' class="button"><%=messages.getText("generic.data.behavtest")%></a>
		</div>
		<div class="col-6"></div>
		<div class="col-7"></div>
		<%
		...
		%>
		<div class="col-8">
			<label for="r1"><img id="left_btn"
				src="images/mutual_left.png" alt="left"></label>
		</div>
		<div class="col-9"></div>
		<div class="col-10">
			<label for="r2"><img id="right_btn"
				src="images/mutual_right.png" alt="right"></label>
		</div>
		<%
			}
		%>
		...
		<tr>
			<td>
				<form action="<%=link%>" method="post" class="searchText">
					...
					<input type="text" name="since_day"
						style="width: 45px; height: 25px" class="input"
						value='<%=since_day%>' placeholder="dd" /> / <input type="text"
						name="since_month" style="width: 45px; height: 25px" class="input"
						value='<%=since_month%>' placeholder="mm" /> / <input type="text"
						name="since_year" style="width: 60px; height: 25px" class="input"
						value='<%=since_year%>' placeholder="yyyy" /> <span
						style="width: 30px; padding-left: 10px; padding-right: 10px;"><%=messages.getText("generic.messages.to")%></span>
					<input type="text" name="until_day"
						style="width: 45px; height: 25px" class="input"
						value='<%=until_day%>' placeholder="dd" /> / <input type="text"
						name="until_month" style="width: 45px; height: 25px" class="input"
						value='<%=until_month%>' placeholder="mm" /> / <input type="text"
						name="until_year"
						style="width: 60px; height: 25px; margin-right: 10px;"
						class="input" value='<%=until_year%>' placeholder="yyyy" /> <input
						name="button"
						style="margin-left: 5px; padding-left: 10px; height: 25px"
						type="submit"
						value='<%=messages.getText("generic.messages.search")%>'
						class="input" />
				</form>
			</td>
		</tr>
	...
	<div class="slidershow middle">
		...
		<div class="slides">
			<input type="radio" name="r" id="r1" checked> <input
				type="radio" name="r" id="r2">
			<table class=" slide table">
				<tr>
					<th><div style="display: flex; align-items: center;">
							<a href='<%=refresh + "&sort=firstname"%>'><img
								onclick="sortBy('firstName')" src="images/mutual_filter.png"
								alt="filter"></a><span class="thText"><%=messages.getText("report.users.name")%></span>
						</div></th>
					<th><div style="display: flex; align-items: center;">
							<a href='<%=refresh + "&sort=lastname"%>'><img
								onclick="sortBy('lastName')" src="images/mutual_filter.png"
								alt="filter"></a><span class="thText"><%=messages.getText("user.data.lastname")%></span>
						</div></th>
					...
					%>
					<th><span class="thText"
						style="display: flex; align-items: center;"><a
							href="/assesment/assesmentReport.jsp?sort=division"><img
								src="images/mutual_filter.png" alt="filter"></a>Centro de
							Costo</span></th>
					<%
						}
							if (Integer.parseInt(assesmentId) == AssesmentData.MUTUAL_DA) {
					%>
					<th><div style="display: flex; align-items: center;">
							<a href="/assesment/assesmentReport.jsp?sort=module1"><img
								src="images/mutual_filter.png" alt="filter"></a><span
								class="thText"><%=messages.getText("assesment1613.module4354.name")%></span>
						</div></th>
					<th><div style="display: flex; align-items: center;">
							<a href="/assesment/assesmentReport.jsp?sort=module2"><img
								src="images/mutual_filter.png" alt="filter"></a><span
								class="thText"><%=messages.getText("assesment1613.module4356.name")%></span>
						</div></th>
					<th><div style="display: flex; align-items: center;">
							<a href="/assesment/assesmentReport.jsp?sort=module3"><img
								src="images/mutual_filter.png" alt="filter"></a><span
								class="thText"><%=messages.getText("assesment1613.module4355.name")%></span>
						</div></th>
					<th><div style="display: flex; align-items: center;">
							<a href="/assesment/assesmentReport.jsp?sort=module4"><img
								src="images/mutual_filter.png" alt="filter"></a><span
								class="thText"><%=messages.getText("assesment1613.module4357.name")%></span>
						</div></th>
					<%
					...
					%>
					<th><div style="display: flex; align-items: center;">
							<a href='<%=refresh + "&sort=module" + cont%>'><img
								src="images/mutual_filter.png" alt="filter"></a><span
								class="thText"><%=messages.getText("assesment" + assesmentId + ".module" + mod.getId() + ".name")%></span>
						</div></th>
					...
					<th><div style="display: flex; align-items: center;">
							<a href='<%=refresh + "&sort=behaviour"%>'><img
								src="images/mutual_filter.png" alt="filter"></a><span
								class="thText"><%=messages.getText("assessment.psi")%>
						</div></th>
					<th><div style="display: flex; align-items: center;">
							<span class="thText"><%=messages.getText("question.type.date")%></span>
						</div></th>
					<th><div style="display: flex; align-items: center;">
							<a href='<%=refresh + "&sort=ranking"%>'><img
								src="images/mutual_filter.png" alt="filter"></a><span
								class="thText"><%=messages.getText("generic.data.ranking")%>(%)</span>
						</div></th>
				...
				<tr class='<%=(odd) ? "odd" : ""%>'>
					...
					<%
						if (cedi == null && Integer.parseInt(assesmentId) == AssesmentData.MUTUAL_DA) {
										String loc = result.getLocation() == null ? "--"
												: sys.getCorporationReportFacade()
														.findCedi(Integer.parseInt(result.getLocation()), userSessionData)
														.getName();
					%>
					<td><%=loc%></td>
					<%
					...
					<%
						if (Integer.parseInt(assesmentId) != AssesmentData.MUTUAL_DA
											&& Integer.parseInt(assesmentId) != AssesmentData.GUINEZ_INGENIERIA_V3) {
					%>
					...
					<%
									...
									if (assesment.getId() == AssesmentData.MUTUAL_DA
											|| assesment.getId() == AssesmentData.ABBOTT_NEWDRIVERS
											|| assesment.getId() == AssesmentData.SUMITOMO) {
					...
					<%
						if (Integer.parseInt(assesmentId) == AssesmentData.ABBOTT_NEWDRIVERS) {
										...
										if (!result.getBehaviourCompleted() || !result.getModule1Completed()
												|| !result.getModule2Completed() || !result.getModule3Completed()
												|| !result.getModule4Completed() || !result.getModule6Completed()
												|| !result.getModule6Completed()) {
					%>
					<td style="text-align: center;"><%=messages.getText("generic.uncompleted")%></td>
					<%
						} else {
					...
					<%
											...
											|| Integer.parseInt(assesmentId) == AssesmentData.SUMITOMO) {
										if (!result.getBehaviourCompleted() || !result.getModule1Completed()
												|| !result.getModule2Completed() || !result.getModule3Completed()
			...
			<div class="slide">
				<table class="table2">
					<tr>
						...
						<%
							if (cedi == null && assesment.getId() != AssesmentData.ABBEVIE_LATAM
											&& assesment.getId() != AssesmentData.SUMITOMO
											&& assesment.getId() != AssesmentData.GUINEZ_INGENIERIA_V3) {
						%>
						<th><span class="thText"><%=messages.getText("generic.data.company")%></span></th>
						<%
							}
						...
						%>
						<th><div style="display: flex; align-items: center;">
								<a target="_blank" href='<%=rec0%>'><img
									src="images/mutual_info.png" alt="filter"></a><span
									class="thText"><%=messages.getText("assesment" + assesmentId + ".module" + mod.getId() + ".name")%>
									<%=messages.getText("generic.data.recommendation")%></span>
							</div></th>
						<%
							}
						%>
						...
						%><th><div style="display: flex; align-items: center;">
								<a target="_blank" href='<%=rec0%>'><img
									src="images/mutual_info.png" alt="info"></a><span
									class="thText"><%=messages.getText("assessment.psi")%> <%=messages.getText("generic.data.recommendation")%></span>
							</div></th>
					...
					<tr class='<%=(odd) ? "odd" : ""%>'>
						...
						<td><%=result.getLogin().toLowerCase()%></td>
						<%
							if (cedi == null && assesment.getId() != AssesmentData.GUINEZ_INGENIERIA_V3
												&& assesment.getId() != AssesmentData.ABBEVIE_LATAM
												&& assesment.getId() != AssesmentData.SUMITOMO) {
											String loc = result.getLocation() == null ? "--"
													: sys.getCorporationReportFacade()
															.findCedi(Integer.parseInt(result.getLocation()), userSessionData)
						...
						%>
						<td style="color: #da2a2a;"><a target="_blank"
							href='<%=rec1%>'><%=messages.getText(result.getMod1Recommendation())%></td>
						...
						%>
						<td style="color: #da2a2a;"><a target="_blank"
							href='<%=rec2%>'><%=messages.getText(result.getMod2Recommendation())%></td>
						...
						%>
						<td style="color: #da2a2a;"><a target="_blank"
							href='<%=rec3%>'><%=messages.getText(result.getMod3Recommendation())%></td>
						...
						%>
						<td style="color: #da2a2a;"><a target="_blank"
							href='<%=rec4%>'><%=messages.getText(result.getMod4Recommendation())%></td>
						...
						%>
						<td style="color: #da2a2a;"><a target="_blank"
							href='<%=rec5%>'><%=messages.getText("generic.data.apply")%></td>
