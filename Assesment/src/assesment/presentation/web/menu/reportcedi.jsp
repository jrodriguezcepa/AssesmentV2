...
<%
	...
	boolean check = false;
	String cedi = request.getParameter("cedi");
	...
	if(role.equals(SecurityConstants.ADMINISTRATOR) || role.equals(SecurityConstants.CLIENTGROUP_REPORTER)) {
		...
		int idGroup = 14;
		int colspan = 4;
		Collection<UserData> users = usReport.findCediUsers(new Integer(cedi), sys.getUserSessionData());
...
%>
      ...
      // Load the Visualization API and the piechart package.
		...
		function associate(user, type) {
			var form = document.forms['CediAsociateAssessmentForm'];
			form.login.value = user;			
			if(type == 3) {
				if(cofirm('Confirma que desea asociar eBTW al usuario')) {
	...
	<body>
		...
		<div id="rcorners2">
            <div> 
                <input class="button"  type="button" value="Descargar" onclick="javascript:document.forms['DownloadCediReportForm'].submit()">
