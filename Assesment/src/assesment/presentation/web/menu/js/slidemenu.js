...
function b_bgImg(img){
	...
	if(document.getElementById){
		...
		imagen=document.getElementById(this.id+"_img");
	}
	else 
	if(document.layers) {
		this.ref.layers[0].document.images[0].src = img
		imagen=this.ref.layers[0].document.images[0];
	}	
	else{ 
		document.images[this.id+"_img"].src = img
		imagen=document.images[this.id+"_img"];
	}
}
...
createSlideMenu.prototype.init = function(){
	...
	for(var j=0;j<this.topmenus.length;j++){
		...
		yy+=el.o.h + this.l[0].between;
		el.o.clipTo(0,this.l[0].width,el.o.h,0,1)
		if(j!=this.topmenus.length-1)
			el.nexttop = this.topmenus[el.top+1] 
		this.loop(el.num,j)
	}
	this.cont.showIt(); 
	this.cont = null; 
...
********************************************************************************/
createSlideMenu.prototype.heightMenu = function(){
	var max=0;
	for(i=0;i<this.menus.length;i++){
			max+=this.l[this.menus[i].lev].height;
	}
	return max;	
}
...
createSlideMenu.prototype.switchMenu = function(num){
	//En fierfox setTimeout 150	
   //setTimeout('parent.refactor.refactorMenu(200)',150);

    
	if(this.going) return
	var m = this.menus[num]
	var active=this.active[m.lev]
	...
	if(active!=num){
		...
		this.desactivarMenu(m.nextel!=-1?m.nextel:m.nexttop,num,y);
	}
}
/********************************************************************************
Move items up
	...
	if(move) setTimeout(this.name+".moveUp("+top+","+num+","+y+")",this.timspeed)
		...
		for(var i=m.lev;i<this.active.length;i++){ //Reset images - actives et cetera
			if(this.active[i]>-1){
				...
				if(this.useImages) 
					temp.o.bgImg(temp.img)//Change bg
				if(this.arrow) 
		...
		if(m.nexttop>-1 || m.nextel>-1)
			 this.moveDown(m.nexttop,m.nextel,y,yy) //Then start moving all other elements down
		else this.going=0
...
Desactive item
********************************************************************************/
createSlideMenu.prototype.desactivarMenu = function(top,num,y){
	m=this.menus[num];
	active=this.active[m.lev]
	var move = 0, m, yy=0;
	if( this.active[0]>-1 && top>-1){
	...
	if(move) setTimeout(this.name+".desactivarMenu("+top+","+num+","+y+")",this.timspeed)
	else{
		m = this.menus[num]
...
createSlideMenu.prototype.makeMenu = function(type,text,lnk,target,end){
  ...
  if(m.lev>=this.l.length){//Level control (same as CM4)
    ...
    if(m.lev>=this.level.length)
		 p1=this.l[this.level.length-1];
	...
	if(parent){ //Find parent element
		...
		if(this.arrow && parent.childs.length==1){
			str='<span class="'+this.arrow+'">0</span>'
			if(document.getElementById && document.getElementById(parent.id).childNodes){
				...
				parent.ar.innerHTML = str+parent.ar.innerHTML;
				parent.ar=parent.ar.childNodes[0]
			} 
			else this.arrow=""
		}
	...
	if(!this.writesubsonly || type!="top"){
		...
		if(this.useImages){
			...
			str+='<a href="'+lnk+'" '+tg+' onclick="'+this.name+'.switchMenu('+this.curr+');this.blur(); " >'
			str+='<img id="'+id+'_img" border="0" src="'+this.l[m.lev].regImage+'"/>' 
			str+='</a>'
			str+='</div>' //bgdiv
		}
...
function slideMenu_makeLevel(){
  var c=this, a=arguments; 
	...
	c.height=a[2]||null;
	c.between=a[3]||null; 
	c.className=a[4]||null; 
	c.classNameA=a[5]||null;
	c.regImage=a[6]||null; 
	c.roundImg=a[7]||null; 
	c.roundImg2=a[8]||null; 
	c.subImg=a[9]||null; 
	c.subRound=a[10]||null;
	 return c
}
/********************************************************************************
Preloading images
